# llama.cpp formal-v3 raw-token runner

This v3 runner is intentionally isolated from `benchmarks/llama_cpp/`. The
legacy source and CMake project remain byte-for-byte frozen because checked-in
v1/v2 diagnostic receipts bind their hashes. Build formal-v3 binaries only
from this directory; historical diagnostic reproduction continues to use the
legacy directory.
...
With no `--mode`, the runner preserves the legacy v2 contract: it requests
`n_ctx=142`, `n_batch=n_ubatch=13`, and `n_seq_max=1`. Some llama.cpp/model
combinations round that legacy context upward; the JSON preserves both values
and rejects an effective context smaller than 142. `--mode native-v3-free`
uses the same free-running workload but requires requested and effective
context sizes to equal 256. In both free modes, the first
`token_ready_elapsed_ns` value includes prompt decode and greedy selection;
later values are cumulative from the same origin. The 128th selected token is
intentionally not decoded, so `n_eval` is 127 while `output.token_ids` contains
128 IDs.

`--mode native-v3-teacher` is the independent quality-admission lane. It
prefills only the first 12 raw prompt tokens, then decodes 128 fixed teacher
inputs: prompt token 13 followed by the first 127 IDs of the frozen free-run
trajectory. Each step scans all 248,320 logits with the same lowest-token-ID
tie rule, records the observed and reference IDs plus mismatch positions, and
timestamps only after full-vocabulary argmax. A mismatch is published as a
complete `ok=false` receipt and returns a nonzero status. This mode is not a
performance sample.
...
```sh
cmake -S benchmarks/llama_cpp_v3 -B /tmp/apxinf-llama-runner-build \
...
Use `--mode native-v3-teacher` for the prompt12 plus 128-step quality receipt.
Omit `--mode` only when reproducing the legacy v2 diagnostic contract.
