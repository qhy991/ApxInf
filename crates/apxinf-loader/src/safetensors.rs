...
/// Load selected tensors as read-only views into SafeTensors file mappings.
/// Payload bytes are not copied; callers must treat the checkpoint files as
/// immutable for the lifetime of the returned tensors.
pub fn load_native_path_mmap_filtered<F>(
    path: &Path,
    include: F,
) -> Result<(HashMap<String, Tensor>, HashMap<String, String>), String>
where
    F: Fn(&str) -> bool,
{
    load_native_path_mmap_filtered_ref(path, &include)
}
...
fn load_native_path_mmap_filtered_ref(
    path: &Path,
    include: &dyn Fn(&str) -> bool,
) -> Result<(HashMap<String, Tensor>, HashMap<String, String>), String> {
    if path.is_dir() {
        let index = path.join("model.safetensors.index.json");
        if index.is_file() {
            return load_native_sharded_mmap_filtered_ref(&index, include);
        }
        let model = path.join("model.safetensors");
        if model.is_file() {
            return load_native_mmap_filtered_ref(&model, include);
        }
        let mut candidates = std::fs::read_dir(path)
        ...
        candidates.sort();
        return match candidates.as_slice() {
            [only] => load_native_mmap_filtered_ref(only, include),
            [] => Err(format!(
                "no SafeTensors model or index in {}",
                path.display()
            )),
    ...
    if path.extension().and_then(|value| value.to_str()) == Some("json") {
        load_native_sharded_mmap_filtered_ref(path, include)
    } else {
        load_native_mmap_filtered_ref(path, include)
    }
}
...
fn load_native_sharded_filtered_ref(
    ...
    load_native_sharded_filtered_mode(index_path, include, false)
}

fn load_native_sharded_mmap_filtered_ref(
    index_path: &Path,
    include: &dyn Fn(&str) -> bool,
) -> Result<(HashMap<String, Tensor>, HashMap<String, String>), String> {
    load_native_sharded_filtered_mode(index_path, include, true)
}

fn load_native_sharded_filtered_mode(
    index_path: &Path,
    include: &dyn Fn(&str) -> bool,
    mapped_payloads: bool,
) -> Result<(HashMap<String, Tensor>, HashMap<String, String>), String> {
    let raw = std::fs::read_to_string(index_path)
        .map_err(|e| format!("failed to read {}: {e}", index_path.display()))?;
    let index: SafetensorsIndex = serde_json::from_str(&raw)
    ...
    for (shard, selected_names) in selected_by_shard {
        ...
        let (shard_tensors, shard_metadata) = if mapped_payloads {
            load_native_mmap_filtered_ref(&parent.join(shard), &select_from_shard)?
        } else {
...
fn load_native_mmap_filtered_ref(
    path: &Path,
    include: &dyn Fn(&str) -> bool,
) -> Result<(HashMap<String, Tensor>, HashMap<String, String>), String> {
    let file =
    ...
    let mmap =
        Arc::new(unsafe { Mmap::map(&file).map_err(|error| format!("mmap failed: {error}"))? });
    load_impl_filtered_mapping(mmap, false, include, true)
}
...
fn load_impl_filtered_file(
    ...
    load_impl_filtered_mapping(mmap, upcast_bf16, include, false)
}

fn load_impl_filtered_mapping(
    mmap: Arc<Mmap>,
    upcast_bf16: bool,
    include: &dyn Fn(&str) -> bool,
    mapped_payloads: bool,
    ...
    for (name, value) in &raw {
        ...
        let tensor = if dtype == DType::BF16 && upcast_bf16 {
            let bf16_bytes: &[u8] = &mmap[abs_start..abs_end];
        ...
        } else if mapped_payloads {
            Tensor::from_mmap(shape, dtype, mmap.clone(), abs_start)
                .map_err(|e| format!("tensor '{name}' mmap construction error: {e}"))?
        } else {
            Tensor::from_raw(shape, dtype, Device::Cpu, mmap[abs_start..abs_end].to_vec())
...
mod tests {
    ...
    #[test]
    fn mmap_filtered_single_file_keeps_payload_read_only_and_zero_copy() {
        let data = [1.25f32, -3.5]
            .iter()
            .flat_map(|value| value.to_le_bytes())
            .collect::<Vec<_>>();
        let mut tmp = NamedTempFile::new().unwrap();
        tmp.write_all(&make_safetensors(&[(
        ...
        let (mut tensors, _) = load_native_path_mmap_filtered(tmp.path(), |_| true).unwrap();
        let tensor = tensors.get_mut("model.language_model.weight").unwrap();
        assert!(matches!(
            tensor.storage(),
            apxinf_core::Storage::CpuMmap { .. }
        ));
        assert_eq!(tensor.as_f32().unwrap(), &[1.25, -3.5]);
        assert!(tensor.as_f32_mut().is_err());
    }

    #[test]
    fn mmap_filtered_sharded_index_maps_only_selected_payloads() {
        let directory = tempfile::tempdir().unwrap();
        std::fs::write(
            directory.path().join("model-00001-of-00002.safetensors"),
            make_safetensors(&[("keep", DType::F32, &[1], &2.5f32.to_le_bytes())]),
        )
        .unwrap();
        std::fs::write(
            directory.path().join("model-00002-of-00002.safetensors"),
            make_safetensors(&[("skip", DType::F32, &[1], &9.0f32.to_le_bytes())]),
        )
        .unwrap();
        std::fs::write(
            directory.path().join("model.safetensors.index.json"),
            r#"{"weight_map":{"keep":"model-00001-of-00002.safetensors","skip":"model-00002-of-00002.safetensors"}}"#,
        )
        .unwrap();

        let (tensors, _) =
            load_native_path_mmap_filtered(directory.path(), |name| name == "keep").unwrap();
        assert_eq!(tensors.len(), 1);
        assert!(matches!(
            tensors["keep"].storage(),
            apxinf_core::Storage::CpuMmap { .. }
        ));
