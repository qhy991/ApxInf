    ...
    pub fn from_map(cfg: &Qwen3VLConfig, mut tensors: HashMap<String, Tensor>) -> Result<Self> {
        ...
        let patch_embed_weight = {
            ...
            let patch_width = vc
                ...
                .and_then(|value| value.checked_mul(vc.patch_size))
                .ok_or_else(|| Error::Other("vision patch width overflow".into()))?;
            // HF stores [hidden, channels, temporal, patch, patch]. Flatten
            // then transpose to [patch_width, hidden] for row-major matmul.
