    ...
    pub fn from_map(cfg: &Qwen3VLConfig, mut tensors: HashMap<String, Tensor>) -> Result<Self> {
        ...
        for i in 0..depth {
            ...
            blocks.push(Qwen3VLVisionBlock {
                ...
                proj_w:  reshape_linear_weight(&take(&format!("{p}.attn.proj.weight"), &mut tensors)?)?,
                ...
                fc1_w:   reshape_linear_weight(&take(&format!("{p}.mlp.linear_fc1.weight"), &mut tensors)?)?,
                fc1_b:   take(&format!("{p}.mlp.linear_fc1.bias"), &mut tensors)?,
                fc2_w:   reshape_linear_weight(&take(&format!("{p}.mlp.linear_fc2.weight"), &mut tensors)?)?,
        ...
        let pos_embed = tensors.remove("model.visual.pos_embed.weight")
...
fn reshape_5d_to_2d(t: &Tensor, rows: usize, cols: usize) -> Result<Tensor> {
    ...
    if expected != rows * cols {
        return Err(Error::Other(format!(
            "reshape_5d_to_2d: {} elements != {}x{}", expected, rows, cols)));
    }
    match t.dtype() {
        ...
        dtype => Err(Error::Other(format!("Qwen3-VL vision reshape does not support {dtype}"))),
    }
}
...
fn transpose_2d(tensor: &Tensor) -> Result<Tensor> {
    ...
    if dims.len() != 2 {
        return Err(Error::Other(format!(
            "transpose_2d expected 2D, got {}D", dims.len())));
    }
    ...
    match tensor.dtype() {
        ...
        dtype => Err(Error::Other(format!("Qwen3-VL vision transpose does not support {dtype}"))),
    }
}
...
pub fn transfer_vision_weights(w: &Qwen3VLVisionWeights, backend: &dyn apxinf_core::Backend) -> Result<Qwen3VLVisionWeights> {
    let transfer_block = |b: &Qwen3VLVisionBlock| -> Result<Qwen3VLVisionBlock> {
        Ok(Qwen3VLVisionBlock {
            ...
            norm2_w: backend.to_device(&b.norm2_w)?, norm2_b: backend.to_device(&b.norm2_b)?,
            fc1_w:   backend.to_device(&b.fc1_w)?,   fc1_b:   backend.to_device(&b.fc1_b)?,
