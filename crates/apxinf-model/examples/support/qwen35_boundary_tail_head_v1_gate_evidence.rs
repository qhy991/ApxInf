...
const FROZEN_GENERAL_SOURCE_SHA256: &str =
    "26d6c010f0bb7f8f407643ee9fd9a5eb0499962152d200c56489ebd9475779dc";
...
const MLP_STACK3_BOUNDARY_RUST_SOURCE_BYTES: &[u8] =
...
const METAL_W8_LINEAR_LAYER_BRIDGE_SOURCE_BYTES: &[u8] =
    include_bytes!("../../../apxinf-metal/src/metal_w8_linear_layer_bridge.mm");
const STACK3_BRIDGE_SOURCE_BYTES: &[u8] =
    include_bytes!("../../../apxinf-metal/src/metal_w8_linear_layer_stack3_bridge.mm");
const MLP_STACK3_BOUNDARY_BRIDGE_SOURCE_BYTES: &[u8] =
...
fn boundary_tail_head_v1_source_set_specs() -> [BuildSourceSpec; 50] {
    [
        BuildSourceSpec {
            ...
            label: "boundary-tail v1 gate evidence source",
        ...
        BuildSourceSpec {
            receipt_name: "gdn_recurrent_profile_rust",
            label: "Metal GDN recurrent profile Rust source",
            manifest_relative_path: "../apxinf-metal/src/gdn_recurrent_profile_v1.rs",
            embedded_bytes: GDN_RECURRENT_PROFILE_RUST_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
        ...
        BuildSourceSpec {
            receipt_name: "mlp_stack3_boundary_rust",
            label: "Metal MLP to Stack3 boundary Rust source",
            manifest_relative_path: "../apxinf-metal/src/linear_layer/mlp_stack3_boundary.rs",
            embedded_bytes: MLP_STACK3_BOUNDARY_RUST_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
            receipt_name: "tail_mlp_head_v1_rust",
            label: "Metal tail MLP head v1 Rust source",
            manifest_relative_path: "../apxinf-metal/src/tail_mlp_head_v1.rs",
            embedded_bytes: TAIL_MLP_HEAD_V1_RUST_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
        ...
        BuildSourceSpec {
            receipt_name: "metal_w8_gdn_bridge",
            label: "Metal W8 GDN bridge source",
            manifest_relative_path: "../apxinf-metal/src/metal_w8_gdn_bridge.mm",
            embedded_bytes: METAL_W8_GDN_BRIDGE_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
            receipt_name: "metal_w8_linear_layer_bridge",
            label: "Metal W8 linear-layer bridge source",
            manifest_relative_path: "../apxinf-metal/src/metal_w8_linear_layer_bridge.mm",
            embedded_bytes: METAL_W8_LINEAR_LAYER_BRIDGE_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
        ...
        BuildSourceSpec {
            receipt_name: "mlp_stack3_boundary_bridge",
            label: "Metal MLP to Stack3 boundary bridge source",
            manifest_relative_path: "../apxinf-metal/src/metal_w8_mlp_stack3_boundary_v1_bridge.mm",
            embedded_bytes: MLP_STACK3_BOUNDARY_BRIDGE_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
            receipt_name: "tail_mlp_head_v1_bridge",
        ...
        BuildSourceSpec {
            ...
            label: "Metal W8 head shader source",
            manifest_relative_path: "../apxinf-metal/src/metal_w8.metal",
            embedded_bytes: METAL_W8_HEAD_SOURCE_BYTES,
            metal_shader: true,
        },
        BuildSourceSpec {
            receipt_name: "gdn_recurrent_count18_profile_bridge",
            label: "Metal GDN recurrent count-18 profile bridge source",
            manifest_relative_path:
                "../apxinf-metal/src/metal_gdn_recurrent_count18_profile_v1_bridge.mm",
            embedded_bytes: GDN_RECURRENT_COUNT18_PROFILE_BRIDGE_SOURCE_BYTES,
            metal_shader: false,
        },
        BuildSourceSpec {
...
fn validate_source_specs(specs: &[BuildSourceSpec]) -> Result<(), Box<dyn Error>> {
    ...
    let observed_names = specs
        ...
        .collect::<BTreeSet<_>>();
    if specs.len() != EXPECTED_NAMES.len()
        || observed_names.len() != specs.len()
    ...
    let build_source = std::str::from_utf8(APXINF_METAL_BUILD_SOURCE_BYTES)
        .map_err(|_| invalid("apxinf-metal build source is not UTF-8"))?;
    let declared_build_inputs = build_source
        .lines()
        .filter_map(|line| {
            let marker = "cargo:rerun-if-changed=src/";
            let start = line.find(marker)? + marker.len();
            let suffix = line[start..].split('"').next()?;
            Some(format!("../apxinf-metal/src/{suffix}"))
        })
        .collect::<BTreeSet<_>>();
    let attested_build_inputs = specs
        .iter()
        .filter(|spec| {
            spec.manifest_relative_path.ends_with(".mm")
                || spec.manifest_relative_path.ends_with(".metal")
        })
...
impl GateCustody {
    pub fn capture_boundary_tail_head_v1(
        model_dir: &Path,
    ...
    fn capture_inner(
        ...
        for spec in source_specs_for_set(source_set_id)? {
            ...
            if spec.receipt_name == "general"
                && source.attestation.sha256 != FROZEN_GENERAL_SOURCE_SHA256
            {
                return Err(invalid(format!(
                    "Qwen3.5 general source SHA-256 drifted: expected {FROZEN_GENERAL_SOURCE_SHA256}, got {}",
    ...
    pub fn receipt_json(&self) -> Value {
        ...
        let sources = json!({
            ...
            "compiled_metal_shader_sources": metal_shader_sources,
        });
        json!({
            "profile": {
...
mod tests {
    ...
    #[test]
    fn boundary_tail_head_v1_capture_is_a_separate_public_custody_entrypoint() {
        let capture: fn(&Path, &Path, &str, &[u8]) -> Result<GateCustody, Box<dyn Error>> =
            GateCustody::capture_boundary_tail_head_v1;
        let _ = capture;
    }
    ...
    fn boundary_tail_v1_explicit_source_set_binds_every_declared_build_input_once() {
        ...
        assert_eq!(
            format!("{:x}", Sha256::digest(GENERAL_SOURCE_BYTES)),
            FROZEN_GENERAL_SOURCE_SHA256
        );
    }
    ...
    fn boundary_tail_v1_rejects_legacy_unknown_and_incomplete_source_sets() {
        ...
        let mut duplicate = specs.to_vec();
        duplicate.push(specs[0]);
        assert!(validate_source_specs(&duplicate).is_err());
    }
