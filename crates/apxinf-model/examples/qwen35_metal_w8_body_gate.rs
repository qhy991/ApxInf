//! Two-process teacher-forced correctness gate for one Metal W8 body layer.
...
const FORMAT: &str = "apxinf-qwen35-metal-w8-body-teacher-gate-v1";
const BODY_LAYER_SET_FORMAT: &str = "apxinf-qwen35-metal-w8-body-layer-set-teacher-gate-v2";
...
fn usage() -> &'static str {
    "Usage: qwen35_metal_w8_body_gate \
  --model-dir PATH \
  --mode cpu|body \
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut model = match args.mode {
        ...
        Mode::Body => GeneralQwen35::from_weights_with_metal_w8_body_layers(
            config,
            tensors,
            Device::Cpu,
            max_context,
    ...
    match args.mode {
        ...
        Mode::Body => {
            let teacher_path = args
                .teacher_json
                .as_ref()
                .ok_or("--teacher-json is required in body mode")?;
            ...
            println!(
                "{}",
                serde_json::to_string(&json!({
                    "format": BODY_LAYER_SET_FORMAT,
                    "mode": "metal_w8_body_teacher_forced",
                    "model_dir": canonical_model_dir,
                    "prompt": args.prompt,
                    "prompt_token_ids": prompt_tokens,
                    "prefill_token": prefill_token,
                    ...
                    "teacher_input_ids": teacher_inputs,
                    "cpu_expected_output_ids": expected_outputs,
                    "metal_body_actual_output_ids": actual_outputs,
                    ...
                    "path_receipt": stats.iter().map(|stats| json!({
                        ...
                        "projection_mean_us": stats.projection_elapsed_ns as f64
    ...
    while let Some(raw_flag) = iter.next() {
        ...
        match flag.as_ref() {
            ...
            "--layer" => {
                ...
                if layers.replace(vec![layer]).is_some() || all_layers {
                    return Err("select body layers with exactly one layer flag".into());
                }
            }
            "--layers" => {
                let parsed = parse_layers(&value(&mut iter)?.to_string_lossy())?;
                if layers.replace(parsed).is_some() || all_layers {
                    return Err("select body layers with exactly one layer flag".into());
                }
            }
            "--all-layers" => {
...
fn validate_receipt(
    ...
    {
        return Err("CPU teacher receipt does not match this body-gate request".into());
    }
