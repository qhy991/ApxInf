//! Direct shared-generation trajectory receipt for a Metal W8 body layer set.
...
const FORMAT: &str = "apxinf-qwen35-metal-w8-body-free-run-v1";
...
fn usage() -> &'static str {
    "Usage: qwen35_metal_w8_body_free_run \
  --model-dir PATH \
  --mode cpu|body \
  [--max-new-tokens 128] \
  [--prompt Hello] \
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        ...
        serde_json::to_string(&json!({
            "format": FORMAT,
            "mode": match args.mode { Mode::Cpu => "cpu", Mode::Body => "metal_w8_body" },
            "model_dir": std::fs::canonicalize(&args.model_dir)?,
            "prompt": args.prompt,
            "prompt_token_ids": prompt_tokens,
            "max_new_tokens": args.max_new_tokens,
            "eos_stopping": false,
            "selected_layers": if args.mode == Mode::Body { Some(&selected_layers) } else { None },
