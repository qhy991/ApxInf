...
fn parse_tail_rows_kernel(value: &str) -> Result<TailMlpHeadRowsKernelV1, String> {
    match value {
        ...
        "pair2-r16-sg8" => Ok(TailMlpHeadRowsKernelV1::Pair2R16Sg8),
...
fn official_tail_ledger_is_exact(
    ...
    let (partial_topk_bytes, total_persistent_bytes) = match rows_kernel {
        ...
        TailMlpHeadRowsKernelV1::Pair2R16Sg8 => (496_640, 282_426_384),
    };
    ledger.scope == "resident-mtlbuffer-only"
        && ledger.exclusions
...
fn official_aggregate_ledger_is_exact(
    ...
    let expected_total_persistent_bytes = match rows_kernel {
        ...
        TailMlpHeadRowsKernelV1::Pair2R16Sg8 => 799_046_672,
    };
...
fn boundary_tail_generation_receipt_is_exact(
    ...
    let expected_persistent_mtlbuffer_bytes =
        match tail.get("topk_rows_kernel").and_then(Value::as_str) {
            ...
            Some("w8_rows_topk4_pair2_r16_sg8") => Some(799_046_672),
            _ => None,
        };
...
fn usage() -> &'static str {
  ...
  [--tail-rows-kernel legacy-r8-sg8|pair2-r16-sg8] \
...
mod tests {
    ...
    fn parser_requires_absolute_paths_candidate_input_and_new_output() {
        ...
        let pair2_r16_sg8 = parse_args_from([
            "gate",
            "--model-dir",
            "/model",
            "--source-lock",
        ...
        .unwrap();
        assert_eq!(
            pair2_r16_sg8.tail_rows_kernel,
            TailMlpHeadRowsKernelV1::Pair2R16Sg8
        );
        assert!(parse_args_from([
            "gate",
    ...
    fn component_ledgers_and_recomputed_aggregate_are_strictly_frozen() {
        ...
        let mut pair2_r16_sg8 = aggregate.clone();
        pair2_r16_sg8.tail =
            ...
            .unwrap();
        pair2_r16_sg8.total_persistent_mtlbuffer_bytes = 799_046_672;
        assert!(official_aggregate_ledger_is_exact(
            &pair2_r16_sg8,
            TailMlpHeadRowsKernelV1::Pair2R16Sg8,
        ));
    ...
    fn generation_receipt_locks_six_regions_tail_phases_and_commit_masks() {
        ...
        let mut pair2_r16_sg8 = receipt.clone();
        pair2_r16_sg8["decode_head"]["topk_rows_kernel"] = json!("w8_rows_topk4_pair2_r16_sg8");
        pair2_r16_sg8["decode_head"]["rows_per_threadgroup"] = json!(16);
        pair2_r16_sg8["decode_head"]["rows_per_simdgroup"] = json!(2);
        pair2_r16_sg8["decode_head"]["simdgroups_per_threadgroup"] = json!(8);
        pair2_r16_sg8["decode_head"]["threads_per_threadgroup"] = json!(256);
        pair2_r16_sg8["decode_head"]["partial_count"] = json!(15_520);
        pair2_r16_sg8["decode_head"]["partial_topk_bytes"] = json!(496_640);
        pair2_r16_sg8["aggregate"]["persistent_mtlbuffer_bytes"] = json!(799_046_672);
        assert!(boundary_tail_generation_receipt_is_exact(
            &pair2_r16_sg8,
            7,
            phase
        ));
        let mut wrong = pair2_r16_sg8.clone();
        wrong["decode_head"]["pipeline_thread_execution_width"] = json!(64);
        assert!(!boundary_tail_generation_receipt_is_exact(&wrong, 7, phase));
        let mut wrong = pair2_r16_sg8;
