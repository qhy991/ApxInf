...
The upstream files are licensed under Apache-2.0 and retain their copyright
and license headers. `core/scalar_type.hpp` is an ApxInf compatibility shim
containing only the compile-time type surface needed by the BF16/U4 template;
it does not import the PyTorch exception or tensor ABI.
