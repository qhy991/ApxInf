...
pub const MAX_SPLITS: usize = 160;
...
#[allow(clippy::too_many_arguments)]
pub fn grouped8_split_cta_write(
    ctx: &CudaContext,
    query: &Tensor,
    key_cache: &CudaBuffer,
    value_cache: &CudaBuffer,
    ...
    {
        return Err(Error::Other(
            "Qwen2.5-Omni grouped8 split-CTA tensor contract mismatch".into(),
        ));
    }
    if ctx.caps().sm != 89
        || !matches!(split_count, 96 | 112 | 128 | 144 | 160)
        || bucket_kv_len <= 11_264
        || bucket_kv_len > max_seq_len
        || max_seq_len > i32::MAX as usize
        || !scale.is_finite()
    ...
    {
        return Err(Error::Other(
            "Qwen2.5-Omni grouped8 split-CTA launch contract mismatch".into(),
        ));
    }
    let cache_bytes = KV_HEADS
        .checked_mul(max_seq_len)
        .and_then(|value| value.checked_mul(HEAD_DIM))
        .and_then(|value| value.checked_mul(DType::BF16.size_in_bytes()))
        .ok_or_else(|| Error::Other("Qwen2.5-Omni grouped8 split-CTA cache overflow".into()))?;
    for (name, buffer) in [("key", key_cache), ("value", value_cache)] {
        if buffer.device() != ctx.device_id() || buffer.len() < cache_bytes {
            return Err(Error::Other(format!(
                "Qwen2.5-Omni grouped8 split-CTA {name} cache contract mismatch"
            )));
        }
    }
    if position.device() != ctx.device_id() || position.len() < 4 {
        return Err(Error::Other(
            "Qwen2.5-Omni grouped8 split-CTA position contract mismatch".into(),
        ));
    }
    let query = CudaBuffer::from_tensor(query).map_err(Error::Cuda)?;
    let output = CudaBuffer::from_tensor(output).map_err(Error::Cuda)?;
    unsafe {
        ffi::check_cuda(
            ffi::apxinf_static_qwen25_omni_attention_flash_grouped8_split_cta_bf16(
                query.ptr(),
                key_cache.ptr(),
                value_cache.ptr(),
                workspace.partial_max.ptr(),
