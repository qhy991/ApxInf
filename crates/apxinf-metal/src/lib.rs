...
/// Explicit input-staging selector for the diagnostic Stack3 body kernels.
/// Existing constructors remain fixed to [`Self::LegacyDevice`].
#[repr(u32)]
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum BodyInputStagingV1 {
    #[default]
    LegacyDevice = 0,
    ThreadgroupShared = 1,
}

impl BodyInputStagingV1 {
    ...
    pub const fn expected_gdn_input_function_name(self) -> &'static str {
        match self {
            Self::LegacyDevice => "gdn_w8_input_projection",
            Self::ThreadgroupShared => "gdn_w8_input_projection_tg_shared",
        }
    }

    pub const fn expected_gdn_output_function_name(self) -> &'static str {
        match self {
            Self::LegacyDevice => "gdn_w8_output_projection_g32",
            Self::ThreadgroupShared => "gdn_w8_output_projection_g32_tg_shared",
        }
    }

    pub const fn expected_mlp_gate_up_function_name(self) -> &'static str {
        match self {
            Self::LegacyDevice => "w8_mlp_gate_up",
            Self::ThreadgroupShared => "w8_mlp_gate_up_tg_shared",
        }
    }

    pub const fn expected_mlp_down_function_name(self) -> &'static str {
        match self {
            Self::LegacyDevice => "w8_mlp_down",
            Self::ThreadgroupShared => "w8_mlp_down_tg_shared",
        }
    }
}

impl TryFrom<u32> for BodyInputStagingV1 {
    type Error = MetalW8Error;

    fn try_from(value: u32) -> Result<Self, Self::Error> {
        match value {
            0 => Ok(Self::LegacyDevice),
            1 => Ok(Self::ThreadgroupShared),
            _ => Err(MetalW8Error::new(format!(
                "Metal W8 body input staging selector {value} is invalid; expected 0 or 1"
            ))),
        }
    }
}

/// Fail-closed runtime proof for the selected Stack3 body input staging. The
/// function names are read from the live `MTLFunction`s retained by the
/// bridge; dynamic threadgroup-memory sizes describe the selected pipelines.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct BodyInputStagingRuntimeReceiptV1 {
    pub requested_profile: BodyInputStagingV1,
    pub observed_profile: BodyInputStagingV1,
    pub threads_per_threadgroup: usize,
    pub gdn_input_threadgroup_bytes: usize,
    pub gdn_output_threadgroup_bytes: usize,
    pub mlp_gate_up_threadgroup_bytes: usize,
    pub mlp_down_threadgroup_bytes: usize,
