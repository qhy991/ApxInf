...
// Four SIMD-groups evaluate eight vocabulary rows per threadgroup. Each
...
// w8_rows_topk4; only the number of resident threads changes.
kernel void w8_rows_topk4_pair2_sg4(
    device const char4 *weights [[buffer(0)]],
    device const float *scales [[buffer(1)]],
    device const float4 *hidden [[buffer(2)]],
    device Candidate *partial [[buffer(3)]],
