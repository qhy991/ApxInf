...
// Eight SIMD-groups evaluate sixteen vocabulary rows per threadgroup. Each
...
// w8_rows_topk4, while the first-stage partial list is halved without using a
// 512-thread threadgroup.
kernel void w8_rows_topk4_pair2_r16_sg8(
    device const char4 *weights [[buffer(0)]],
    device const float *scales [[buffer(1)]],
    device const float4 *hidden [[buffer(2)]],
    device Candidate *partial [[buffer(3)]],
