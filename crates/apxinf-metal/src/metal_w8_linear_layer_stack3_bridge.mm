...
void encode_layer(ApxinfMetalW8LinearLayerStack3HandleV1 *handle,
    ...
    set_body_input_threadgroup_memory(
        handle, encoder, handle->mlp_down_dynamic_threadgroup_memory_bytes);
    ...
    @autoreleasepool {
        ...
        *output = nullptr;
        if (!valid_body_input_staging_selector(body_input_staging)) {
            write_error(error_output, error_capacity,
                        "invalid Metal W8 stack3 v1 body-input-staging selector");
            return 1;
        }
        ...
        uint32_t mlp_down_dynamic_bytes = 0;
        if (!checked_f32_threadgroup_bytes(shape.hidden_size,
            ...
            !checked_f32_threadgroup_bytes(shape.intermediate_size,
                                           &mlp_down_dynamic_bytes)) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 dynamic threadgroup byte count overflows");
            return 1;
        }
        ...
        if (!live_body_input_staging_matches(handle)) {
            delete handle;
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 live body-input-staging pipelines or threadgroup resources do not match the requested selector");
            return 1;
        }
...
extern "C" int apxinf_metal_w8_linear_layer_stack3_create_gdn_out_g32_v1(
    ...
    return create_linear_layer_stack3_gdn_out_g32_impl(
        layers, layer_count, kBodyInputStagingLegacyDevice, output,
...
apxinf_metal_w8_linear_layer_stack3_create_gdn_out_g32_with_body_input_staging_v1(
    ...
    size_t error_capacity) {
    return create_linear_layer_stack3_gdn_out_g32_impl(
        layers, layer_count, body_input_staging, output, error_output,
        error_capacity);
}
...
apxinf_metal_w8_linear_layer_stack3_body_input_staging_receipt_v1(
    ...
    @autoreleasepool {
        ...
        if (!write_body_input_staging_receipt(handle, receipt)) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 live body-input-staging receipt is invalid");
            return 1;
        }
        return 0;
    }
    ...
    @autoreleasepool {
        ...
        *receipt = Stack3ExecutionReceiptV1{};
        ...
        if (!live_body_input_staging_matches(handle)) {
            ...
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 live body-input-staging selection changed after create");
            return 1;
        }
