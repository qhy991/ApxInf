...
struct Stack3ExecutionReceiptV1 {
...
static_assert(sizeof(Stack3LayerDescriptorV1) == 152,
              "Metal W8 stack3 layer descriptor ABI changed");
static_assert(sizeof(Stack3StateDescriptorV1) == 64,
              "Metal W8 stack3 state descriptor ABI changed");
static_assert(sizeof(Stack3MutableStateDescriptorV1) == 64,
              "Metal W8 stack3 mutable state descriptor ABI changed");
static_assert(sizeof(Stack3ExecutionReceiptV1) == 368,
...
struct ApxinfMetalW8LinearLayerStack3HandleV1 {
    id<MTLDevice> device;
...
bool fixed_fused_shape(const Stack3LayerDescriptorV1 &layer) {
...
bool live_profile_matches(ApxinfMetalW8LinearLayerStack3HandleV1 *handle) {
...
id<MTLComputePipelineState> selected_gdn_core_pipeline(
    ApxinfMetalW8LinearLayerStack3HandleV1 *handle) {
...
void write_observed_function_chain(
    ApxinfMetalW8LinearLayerStack3HandleV1 *handle,
    Stack3ExecutionReceiptV1 *receipt) {
    if (handle->gdn_core_profile == kFusedProfile) {
...
uint32_t expected_transaction_dispatches(uint32_t profile) {
    return profile == kFusedProfile ? 30 : 39;
}

uint32_t expected_transaction_barriers(uint32_t profile) {
    return profile == kFusedProfile ? 27 : 36;
}
...
void record_dispatch(Stack3ExecutionReceiptV1 *receipt, bool gdn_core,
...
void buffer_barrier(id<MTLComputeCommandEncoder> encoder,
                    Stack3ExecutionReceiptV1 *receipt, bool gdn_core) {
...
bool same_shape(const Stack3LayerDescriptorV1 &left,
                const Stack3LayerDescriptorV1 &right) {
...
bool descriptor_pointers_valid(const Stack3LayerDescriptorV1 &layer) {
...
bool derive_shape(const Stack3LayerDescriptorV1 &layer, Stack3Shape *shape) {
...
void allocate_layer(id<MTLDevice> device, Stack3Layer *output,
                    const Stack3LayerDescriptorV1 &source,
                    const Stack3Shape &shape) {
...
bool layer_buffers_valid(const Stack3Layer &layer) {
...
bool valid_state_counts(const Stack3Layer &layer, uint32_t query_count,
...
void encode_layer(ApxinfMetalW8LinearLayerStack3HandleV1 *handle,
                  Stack3Layer &layer, id<MTLBuffer> input,
                  id<MTLBuffer> output,
    ...
    record_dispatch(receipt, false, 1, kElementThreads);
    ...
    @autoreleasepool {
        if (output == nullptr) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 output handle is null");
            return 1;
        }
        *output = nullptr;
        if (layers == nullptr || layer_count != kStackDepth ||
            ...
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 requires three layers and production profile 0 or 2; fused requires the strict fixed-shape contract");
            return 1;
        }
        Stack3Shape shape{};
        if (!derive_shape(layers[0], &shape)) {
            write_error(error_output, error_capacity,
                        "invalid Metal W8 stack3 v1 packed contract");
            return 1;
        }
        for (uint32_t index = 1; index < kStackDepth; ++index) {
            Stack3Shape candidate{};
            if (!same_shape(layers[0], layers[index]) ||
                !derive_shape(layers[index], &candidate)) {
                write_error(error_output, error_capacity,
                            "Metal W8 stack3 v1 layer shapes or RMS epsilons differ");
                return 1;
            }
        }
        if (require_fixed_shape != 0) {
            for (uint32_t index = 0; index < kStackDepth; ++index) {
                if (!fixed_fused_shape(layers[index])) {
                    write_error(error_output, error_capacity,
                                "Metal W8 stack3 production profile requires H=1024, KH=VH=16, KD=VD=128, conv=4, input_rows=8224, input_groups=16, persistent G32 output_groups=64, fused core-local guard output_groups=32, and rms_norm_eps bits equal to 1e-6f32");
                    return 1;
                }
            }
        }
        ...
        auto handle =
            new (std::nothrow) ApxinfMetalW8LinearLayerStack3HandleV1{};
        if (handle == nullptr) {
            write_error(error_output, error_capacity,
                        "allocate Metal W8 stack3 v1 handle failed");
            return 1;
        }
        ...
        handle->mlp_gate_up = make_private_f32(handle->device, shape.gate_up_rows);
        handle->mlp_activated =
            make_private_f32(handle->device, shape.intermediate_size);
        bool layers_valid = true;
        for (const Stack3Layer &layer : handle->layers) {
            layers_valid = layers_valid && layer_buffers_valid(layer);
        }
        if (handle->queue == nil || !layers_valid || handle->hidden_a == nil ||
            ...
            delete handle;
            write_error(error_output, error_capacity,
                        "allocate persistent Metal W8 stack3 v1 buffers failed");
            return 1;
        }
...
extern "C" int apxinf_metal_w8_linear_layer_stack3_seed_states_v1(
    void *opaque_handle, const Stack3StateDescriptorV1 *states,
    ...
    @autoreleasepool {
        ...
        if (handle == nullptr || states == nullptr || state_count != kStackDepth) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 requires exactly three seed states");
            return 1;
        }
        ...
        for (uint32_t slot = 0; slot < kStackDepth; ++slot) {
            const Stack3StateDescriptorV1 &state = states[slot];
            if (state.query_state == nullptr || state.key_state == nullptr ||
                ...
                write_error(error_output, error_capacity,
                            "invalid Metal W8 stack3 v1 seed state");
                return 1;
            }
        }
        for (uint32_t slot = 0; slot < kStackDepth; ++slot) {
            Stack3Layer &layer = handle->layers[slot];
            const Stack3StateDescriptorV1 &state = states[slot];
...
extern "C" int apxinf_metal_w8_linear_layer_stack3_decode_v1(
    ...
    Stack3ExecutionReceiptV1 *receipt, char *error_output,
    size_t error_capacity) {
    @autoreleasepool {
        if (receipt == nullptr) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 execution receipt is null");
            return 1;
        }
        *receipt = Stack3ExecutionReceiptV1{};
        auto handle =
            static_cast<ApxinfMetalW8LinearLayerStack3HandleV1 *>(opaque_handle);
        ...
        if (!live_profile_matches(handle)) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 live GDN profile identity changed");
            return 1;
        }
        if (handle->production_receipt_enabled) {
            for (const Stack3Layer &layer : handle->layers) {
                if (!live_fixed_shape(layer.gdn_params)) {
                    write_error(error_output, error_capacity,
                                "Metal W8 stack3 production fixed-shape contract changed before dispatch");
                    return 1;
                }
            }
        }
        ...
        if (handle->seeded_mask != kAllSeededMask) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 seed mask is not 111");
            return 1;
        }
        if (!all_finite(input, input_count)) {
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 input is non-finite");
            return 1;
        }
        ...
        if (command == nil) {
            ...
            write_error(error_output, error_capacity,
                        "create Metal W8 stack3 v1 command buffer failed");
            return 1;
        }
        ...
        for (uint32_t slot = 0; slot < kStackDepth; ++slot) {
            ...
            if (encoder == nil) {
                handle->terminal_error = true;
                write_error(error_output, error_capacity,
                            "create Metal W8 stack3 v1 compute encoder failed");
                return 1;
            }
        ...
        if (receipt->kernel_dispatches !=
            ...
            handle->terminal_error = true;
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 pre-commit topology mismatch");
            return 1;
        }
        [command commit];
        receipt->commits = 1;
        ...
        if (inject_failure_after_execution) {
            handle->terminal_error = true;
            write_error(error_output, error_capacity,
                        "injected Metal W8 stack3 v1 failure after scratch execution");
            return 1;
        }
        // This version intentionally has no host checks after layers 0 or 1.
        // Only the final hidden_b row is checked before the all-or-nothing swap.
        const float *final_values =
            static_cast<const float *>(handle->hidden_b.contents);
        if (!all_finite(final_values, input_count)) {
            handle->terminal_error = true;
            write_error(error_output, error_capacity,
                        "Metal W8 stack3 v1 final output is non-finite");
            return 1;
        }
        for (Stack3Layer &layer : handle->layers) {
...
extern "C" int apxinf_metal_w8_linear_layer_stack3_snapshot_state_v1(
    void *opaque_handle, uint32_t slot, Stack3MutableStateDescriptorV1 *state,
    char *error_output, size_t error_capacity) {
    @autoreleasepool {
        auto handle =
            static_cast<ApxinfMetalW8LinearLayerStack3HandleV1 *>(opaque_handle);
        if (handle == nullptr || state == nullptr || slot >= kStackDepth ||
            ...
            !valid_state_counts(handle->layers[slot], state->query_count,
                                ...
                                state->recurrent_count)) {
            write_error(error_output, error_capacity,
                        "invalid Metal W8 stack3 v1 state snapshot");
            return 1;
        }
        Stack3Layer &layer = handle->layers[slot];
...
extern "C" void apxinf_metal_w8_linear_layer_stack3_destroy_v1(
    void *opaque_handle) {
    auto handle =
        static_cast<ApxinfMetalW8LinearLayerStack3HandleV1 *>(opaque_handle);
    delete handle;
