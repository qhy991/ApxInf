...
constexpr uint32_t kHeadPair2R16RowsPerThreadgroup = 16;
constexpr uint32_t kHeadPair2R16RowsPerSimdgroup = 2;
constexpr uint32_t kHeadPair2R16SimdgroupsPerThreadgroup = 8;
constexpr uint32_t kHeadPair2R16Threads =
    kHeadPair2R16SimdgroupsPerThreadgroup * kSimdWidth;
static_assert(kHeadPair2R16RowsPerThreadgroup ==
              kHeadPair2R16RowsPerSimdgroup *
                  kHeadPair2R16SimdgroupsPerThreadgroup);
static_assert(kHeadPair2R16Threads == kMatVecThreads);
...
constexpr uint32_t kRowsKernelPair2R16Sg8 = 1;
...
void encode_tail(ApxinfMetalW8TailMlpHeadHandleV1 *handle,
    ...
    [encoder dispatchThreadgroups:MTLSizeMake(handle->head_params.partial_count,
             ...
             threadsPerThreadgroup:MTLSizeMake(
                 handle->rows_kernel == kRowsKernelPair2R16Sg8
                     ? kHeadPair2R16Threads
                     : kMatVecThreads,
                 1, 1)];
    buffer_barrier(encoder);
    ...
    @autoreleasepool {
        ...
        *output = nullptr;
        if (descriptor == nullptr || descriptor->gate_up_weights == nullptr ||
            ...
            rows_kernel > kRowsKernelPair2R16Sg8 ||
        ...
        if (rows_topk_function != nil) {
            ...
            if ([observed_rows_topk_name isEqualToString:@"w8_rows_topk4"]) {
                handle->rows_kernel = kRowsKernelLegacyR8Sg8;
                           ...
                           isEqualToString:@"w8_rows_topk4_pair2_r16_sg8"]) {
                handle->rows_kernel = kRowsKernelPair2R16Sg8;
            } else {
        ...
        const uint32_t rows_per_threadgroup =
            handle->rows_kernel == kRowsKernelPair2R16Sg8
                ? kHeadPair2R16RowsPerThreadgroup
                : kRowsPerThreadgroup;
...
extern "C" int apxinf_metal_w8_tail_mlp_head_rows_kernel_receipt_v1(
    ...
    @autoreleasepool {
        ...
        *receipt = TailRowsKernelReceiptV1{};
        ...
        const bool pair2_r16_sg8 =
            handle->rows_kernel == kRowsKernelPair2R16Sg8;
        const uint32_t rows_per_threadgroup =
            pair2_r16_sg8 ? kHeadPair2R16RowsPerThreadgroup
                          : kRowsPerThreadgroup;
        receipt->rows_kernel = handle->rows_kernel;
        receipt->rows_per_threadgroup = rows_per_threadgroup;
        receipt->rows_per_simdgroup =
            pair2_r16_sg8 ? kHeadPair2R16RowsPerSimdgroup : 1;
        receipt->simdgroups_per_threadgroup =
            pair2_r16_sg8 ? kHeadPair2R16SimdgroupsPerThreadgroup
                          : kRowsPerThreadgroup;
        receipt->threads_per_threadgroup =
            pair2_r16_sg8 ? kHeadPair2R16Threads : kMatVecThreads;
