...
fn tail_v1_rows_kernel_selector_is_explicit_and_legacy_by_default() {
    ...
    assert_eq!(
        TailMlpHeadRowsKernelV1::Pair2Sg4.receipt_label(),
    ...
    assert_eq!(
        TailMlpHeadRowsKernelV1::Pair2Sg4.execution_shape(),
        (8, 2, 4, 128, false)
    );
}
...
#[cfg(target_os = "macos")]
#[test]
fn metal_tail_pair2_sg4_matches_legacy_and_cpu_for_an_incomplete_final_row_group() {
    let hidden_size = 64;
    let intermediate_size = 64;
    let vocab_size = 11;
    let projection_elements = hidden_size * intermediate_size;
    ...
    let mut pair2 = MetalW8TailMlpHeadV1::from_packed_with_rows_kernel(
        &packed,
    ...
    assert_eq!(pair2.rows_kernel(), TailMlpHeadRowsKernelV1::Pair2Sg4);
    ...
    let pair2_output = pair2.decode(&input).unwrap();
    assert_close(
        &legacy_hidden,
        &expected.normalized_hidden,
        3.0e-3,
    ...
    assert_close(
        pair2_output.normalized_hidden,
        &expected.normalized_hidden,
        3.0e-3,
        "pair2 normalized hidden",
    );
    assert_eq!(legacy_candidates, expected.candidate_token_ids);
    assert_eq!(
        pair2_output.candidate_token_ids,
...
#[cfg(target_os = "macos")]
#[test]
fn metal_tail_pair2_sg4_preserves_lowest_token_ties_with_invalid_tail_rows() {
    let hidden_size = 64;
    let intermediate_size = 64;
    let vocab_size = 9;
    let projection_elements = hidden_size * intermediate_size;
    ...
    assert_eq!(
        pair2.decode(&input).unwrap().candidate_token_ids,
        [0, 1, 2, 3]
    );
}
...
fn tail_v1_bridge_shape_and_shader_custody_match_the_public_contract() {
    ...
    assert!(bridge.contains("@\"w8_rows_topk4_pair2_sg4\""));
    assert!(bridge.contains("kHeadPair2Threads"));
    ...
    assert!(bridge.contains("rows_kernel > kRowsKernelPair2Sg4"));

    let shader = include_str!("../src/metal_w8.metal");
    assert_eq!(
        shader
            .matches("kernel void w8_rows_topk4_pair2_sg4(")
            .count(),
        1
    );
    assert!(shader.contains("constexpr uint rows_per_simdgroup = 2;"));
    assert!(shader.contains("constexpr uint simdgroups_per_threadgroup = 4;"));
    let standalone_bridge = include_str!("../src/metal_w8_bridge.mm");
    assert!(!standalone_bridge.contains("w8_rows_topk4_pair2_sg4"));
