use apxinf_metal::{
    ...
    TailMlpHeadBufferLedgerV1, TailMlpHeadFinalKernelV1, TailMlpHeadRowsKernelV1,
};
...
fn tail_v1_rows_kernel_selector_is_explicit_and_legacy_by_default() {
    ...
    assert_eq!(
        TailMlpHeadRowsKernelV1::Pair2R32Sg16.receipt_label(),
    ...
    assert_eq!(
        TailMlpHeadRowsKernelV1::Pair2R32Sg16.execution_shape(),
        (32, 2, 16, 512, false)
    );
    ...
    .unwrap();
    let pair2_r32_sg16 = TailMlpHeadBufferLedgerV1::from_dimensions_with_rows_kernel(
        1_024,
        3_584,
        248_320,
    ...
    assert_eq!(pair2_r32_sg16.partial_topk_bytes, 248_320);
    assert_eq!(pair2_r32_sg16.total_persistent_bytes, 282_178_064);
    ...
    assert_eq!(
        TailMlpHeadFinalKernelV1::LegacyThreadgroupSerial.receipt_label(),
        "w8_final_topk4"
    );
    assert_eq!(
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8.receipt_label(),
        "w8_final_topk4_simd_hierarchical"
    );
    assert_eq!(
        TailMlpHeadFinalKernelV1::LegacyThreadgroupSerial.execution_shape(),
        (256, 8, 256, 1)
    );
    assert_eq!(
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8.execution_shape(),
        (256, 8, 8, 1)
    );
    assert_eq!(
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8.static_threadgroup_bytes(),
        256
    );
}
...
#[cfg(target_os = "macos")]
#[test]
fn metal_tail_pair2_r32_sg16_matches_legacy_and_cpu_for_an_incomplete_final_row_group() {
    let hidden_size = 64;
    let intermediate_size = 64;
    let vocab_size = 35;
    let projection_elements = hidden_size * intermediate_size;
    ...
    let mut pair2_r32_sg16 = MetalW8TailMlpHeadV1::from_packed_with_rows_kernel(
        &packed,
        TailMlpHeadRowsKernelV1::Pair2R32Sg16,
    )
    .unwrap();
    let mut combined = MetalW8TailMlpHeadV1::from_packed_with_kernels(
        &packed,
        TailMlpHeadRowsKernelV1::Pair2R32Sg16,
    ...
    assert_eq!(
        pair2_r32_sg16.rows_kernel(),
        TailMlpHeadRowsKernelV1::Pair2R32Sg16
    );
    assert_eq!(
        pair2_r32_sg16.final_kernel(),
        TailMlpHeadFinalKernelV1::LegacyThreadgroupSerial,
        "rows-only ABI must remain pinned to the legacy final kernel"
    );
    assert_eq!(
        combined.rows_kernel(),
        TailMlpHeadRowsKernelV1::Pair2R32Sg16
    );
    assert_eq!(
        combined.final_kernel(),
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8
    );
    let observed = pair2_r32_sg16.rows_kernel_receipt();
    assert_eq!(observed.kernel, TailMlpHeadRowsKernelV1::Pair2R32Sg16);
    assert_eq!(observed.rows_per_threadgroup, 32);
    ...
    let final_observed = combined.final_kernel_receipt();
    assert_eq!(
        final_observed.kernel,
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8
    );
    assert_eq!(final_observed.threads_per_threadgroup, 256);
    assert_eq!(final_observed.simdgroups_per_threadgroup, 8);
    assert_eq!(final_observed.stage_one_leader_count, 8);
    assert_eq!(final_observed.stage_one_candidate_slots, 32);
    assert_eq!(final_observed.threadgroup_barriers, 1);
    assert!(final_observed.pipeline_max_total_threads_per_threadgroup >= 256);
    assert_eq!(final_observed.pipeline_thread_execution_width, 32);
    assert!(final_observed.pipeline_static_threadgroup_memory_length >= 256);
    ...
    let pair2_r32_sg16_output = pair2_r32_sg16.decode(&input).unwrap();
    let combined_output = combined.decode(&input).unwrap();
    assert_close(
        &legacy_hidden,
        &expected.normalized_hidden,
        3.0e-3,
    ...
    assert_close(
        pair2_r32_sg16_output.normalized_hidden,
        &expected.normalized_hidden,
        3.0e-3,
        "pair2 r32 sg16 normalized hidden",
    );
    assert_eq!(legacy_candidates, expected.candidate_token_ids);
    assert_eq!(
        pair2_r32_sg16_output.candidate_token_ids,
        expected.candidate_token_ids
    );
    assert_close(
        combined_output.normalized_hidden,
        &expected.normalized_hidden,
        3.0e-3,
        "pair2 r32 sg16 + SIMD-final normalized hidden",
    );
    assert_eq!(
        combined_output.candidate_token_ids,
        expected.candidate_token_ids
    );
}
...
#[cfg(target_os = "macos")]
#[test]
fn metal_tail_pair2_r32_sg16_preserves_lowest_token_ties_with_invalid_tail_rows() {
    let hidden_size = 64;
    let intermediate_size = 64;
    let vocab_size = 33;
    let projection_elements = hidden_size * intermediate_size;
    ...
    let expected = packed.decode_reference(&input).unwrap();
    let mut pair2_r32_sg16 = MetalW8TailMlpHeadV1::from_packed_with_kernels(
        &packed,
        TailMlpHeadRowsKernelV1::Pair2R32Sg16,
        TailMlpHeadFinalKernelV1::SimdHierarchicalSg8,
    )
    ...
    assert_eq!(
        pair2_r32_sg16.decode(&input).unwrap().candidate_token_ids,
        [0, 1, 2, 3]
    );
}
...
#[cfg(target_os = "macos")]
#[test]
fn metal_tail_simd_final_scans_multiple_partial_candidates_per_thread_exactly() {
    let hidden_size = 64;
    let intermediate_size = 64;
    let vocab_size = 4_097;
    let projection_elements = hidden_size * intermediate_size;
    let packed = PackedW8TailMlpHeadV1::new(
        PackedW8MlpBlock::pack_f32(
            &values(projection_elements, 199, 197, 0.04),
            &values(projection_elements, 211, 193, 0.04),
            &values(projection_elements, 223, 191, 0.04),
            hidden_size,
            intermediate_size,
        )
        .unwrap(),
        ...
        1.0e-6,
        PackedW8Rows::pack_f32(
            &values(vocab_size * hidden_size, 227, 18_143, 0.1),
            vocab_size,
            hidden_size,
        )
        .unwrap(),
    ...
    .unwrap();
    let input = values(hidden_size, 229, 181, 0.8);
    let expected = packed.decode_reference(&input).unwrap();
    let mut combined = MetalW8TailMlpHeadV1::from_packed_with_kernels(
        &packed,
        TailMlpHeadRowsKernelV1::Pair2R32Sg16,
    ...
    assert_eq!(combined.rows_kernel_receipt().partial_count, 129);
    assert!(combined.rows_kernel_receipt().partial_count as usize * 4 > 256);
    let actual = combined.decode(&input).unwrap();
    assert_close(
        actual.normalized_hidden,
        &expected.normalized_hidden,
        3.0e-3,
        "SIMD-final multi-candidate normalized hidden",
    );
    assert_eq!(actual.candidate_token_ids, expected.candidate_token_ids);
}
...
fn tail_v1_bridge_shape_and_shader_custody_match_the_public_contract() {
    ...
    assert!(bridge.contains("@\"w8_rows_topk4_pair2_r32_sg16\""));
    assert!(bridge.contains("kHeadPair2R32Threads"));
    assert!(bridge.contains("apxinf_metal_w8_tail_mlp_head_create_with_rows_kernel_v1("));
    assert!(bridge.contains("apxinf_metal_w8_tail_mlp_head_create_with_kernels_v1("));
    assert!(bridge.contains("apxinf_metal_w8_tail_mlp_head_rows_kernel_receipt_v1("));
    assert!(bridge.contains("apxinf_metal_w8_tail_mlp_head_final_kernel_receipt_v1("));
    assert!(bridge.contains("handle->partial_topk.length"));
    assert!(bridge.contains("rows_topk_function.name"));
    assert!(bridge.contains("final_topk_function.name"));
    assert!(bridge.contains("handle->rows_kernel != rows_kernel"));
    assert!(bridge.contains("handle->final_kernel != final_kernel"));
    assert!(bridge.contains("handle->rows_topk_pipeline.threadExecutionWidth"));
    assert!(bridge.contains("uint32_t rows_kernel"));
    assert!(bridge.contains("rows_kernel > kRowsKernelPair2R32Sg16"));
    assert!(bridge.contains("final_kernel > kFinalKernelSimdHierarchical"));
    assert!(bridge.contains("handle->final_topk_pipeline.threadExecutionWidth"));
    assert!(bridge.contains("handle->final_topk_pipeline.staticThreadgroupMemoryLength"));
    assert!(bridge.contains("kFinalKernelLegacySerial, output, error_output"));

    let shader = include_str!("../src/metal_w8.metal");
    assert_eq!(
        shader
            .matches("kernel void w8_rows_topk4_pair2_r32_sg16(")
            .count(),
        1
    );
    assert_eq!(
        shader
            .matches("kernel void w8_final_topk4_simd_hierarchical(")
            .count(),
        1
    );
    for offset in [1, 2, 4, 8, 16] {
        assert!(shader.contains(&format!("merge_top4_shuffle_down(local, lane, {offset});")));
    }
    for offset in [1, 2, 4] {
        assert!(shader.contains(&format!("merge_top4_shuffle_down(leader, lane, {offset});")));
    }
    let hierarchical = shader
        .split("kernel void w8_final_topk4_simd_hierarchical(")
        .nth(1)
        .unwrap();
    assert_eq!(
        hierarchical
    ...
    assert!(hierarchical.contains("candidate.token < params.rows && !isnan(candidate.score)"));
    assert!(shader.contains("if (lane % span == 0 && lane + uint(offset) < 32)"));
    assert!(hierarchical.contains("if (simdgroup == 0)"));
    assert!(shader.contains("constexpr uint rows_per_simdgroup = 2;"));
    assert!(shader.contains("constexpr uint simdgroups_per_threadgroup = 16;"));
    assert!(shader.contains("rows_per_simdgroup * simdgroups_per_threadgroup;"));
    assert!(bridge.contains("kHeadPair2R32RowsPerThreadgroup = 32"));
    assert!(bridge.contains("kHeadPair2R32RowsPerSimdgroup = 2"));
    assert!(bridge.contains("kHeadPair2R32SimdgroupsPerThreadgroup = 16"));
    assert!(bridge.contains("maxTotalThreadsPerThreadgroup"));
    let standalone_bridge = include_str!("../src/metal_w8_bridge.mm");
    assert!(!standalone_bridge.contains("w8_rows_topk4_pair2_r32_sg16"));
