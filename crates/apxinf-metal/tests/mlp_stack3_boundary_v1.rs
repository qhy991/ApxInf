use apxinf_metal::{
    BodyInputStagingV1, GdnDecodeState, GdnDimensions, GdnF32Weights, MetalW8MlpStack3BoundaryV1,
...
#[cfg(target_os = "macos")]
#[test]
fn metal_boundary_v1_threadgroup_input_staging_is_bit_exact_state_exact_and_live_receipted() {
    let (dims, layer0) = layer_fixture(0);
    let (_, layer1) = layer_fixture(1);
    let (_, layer2) = layer_fixture(2);
    let boundary_norm = values(dims.hidden_size, 101, 197, 0.2)
    ...
    let mut candidate = MetalW8MlpStack3BoundaryV1::from_packed_with_body_input_staging_v1(
        &packed,
...
fn boundary_v1_scratch_failure_publishes_no_state_or_output_and_reset_recovers() {
    ...
    for profile in [
        ...
        let mut metal =
            MetalW8MlpStack3BoundaryV1::from_packed_with_body_input_staging_v1(&packed, profile)
                .unwrap();
